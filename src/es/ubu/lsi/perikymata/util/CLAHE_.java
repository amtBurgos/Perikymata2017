...
/**
 * &lsquot;Contrast Limited Adaptive Histogram Equalization&rsquot; as described
 * in
 * 
 * <br />
 * BibTeX:
...
public class CLAHE_ implements PlugIn {
	...
	final public void run(final String arg) {
		...
		synchronized (imp) {
			if (!imp.isLocked())
				...
				IJ.error("The image '" + imp.getTitle()
	...
	/**
	 * Process an {@link ImagePlus} with the static parameters. Create mask and
	 * bounding box from the {@link Roi} of that {@link ImagePlus}.
	 * 
	 * @param imp
	 */
	...
	/**
	 * Process and {@link ImagePlus} with a given set of parameters. Create mask
	 ...
	 * @param slope
	 */
	final static public void run(final ImagePlus imp, final int blockRadius, final int bins, final float slope) {
	 ...
	 * @return
	 */
	final static public BufferedImage run(final ImagePlus imp, final int blockRadius, final int bins, final float slope,
			final java.awt.Rectangle roiBox, final ByteProcessor mask) {
		...
		if (roiBox == null) {
			if (mask == null)
				...
				box = new Rectangle(0, 0, Math.min(imp.getWidth(), mask.getWidth()),
						Math.min(imp.getHeight(), mask.getHeight()));
		} else
			box = roiBox;
