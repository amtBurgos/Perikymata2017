...
class Procesado():
    ...
    #la distancia entre los puntos que forman los segmentos
    # si esta distancia es mayor que segments_distance son segmentos consecutivos, sino son paralelos
    def segmentPoints_distance(self,seg1, seg2):
        """distance between two segments in the plane:
        one segment is (x11, y11) to (x12, y12)
        the other is   (x21, y21) to (x22, y22)
        """
        ...
        # try each of the 4 vertices w/the other segment
        distances = []
        distances.append(self.pointDistance((x11, y11), (x21, y21)))
        distances.append(self.pointDistance((x11, y11), (x22, y22)))
        distances.append(self.pointDistance((x12, y12), (x21, y21)))
        distances.append(self.pointDistance((x12, y12), (x22, y22)))
        return min(distances)
    ...
    # corregida
    def combinaSegmentos2(self,segmentosList):
        #print("combina",segmentosList)
        Xs=list(map(lambda x:[x[0][0],x[1][0]],segmentosList))
        Ys=list(map(lambda x:[x[0][1],x[1][1]],segmentosList))
        xMax=np.max(Xs)
        ...
        points = []
        for seg in segmentosList:
            points.append(seg[0])
            points.append(seg[1])

        dist1=min(list(map(lambda point: self.modulo((point,(xMax,yMax))), points)))
        dist2=min(list(map(lambda point: self.modulo((point,(xMin,yMax))), points)))
        
        #print(min(list(map(lambda point: modulo((point,(xMax,yMax))), points))))
        #print(min(list(map(lambda point: modulo((point,(xMin,yMax))), points))))
        
        #en ocasiones no va a ser exactamente igual, pero no por ello tenemos que cambiar la orientación
        # basta con que la distancia sea menor a cambiando la orientación
        if dist1<dist2: # 
            #print("devuelvo",((xMax,yMax),(xMin,yMin)))
            return ((xMax,yMax),(xMin,yMin))
        else:
    ...
    def segmentosVerdad(self,k_components,lines):
        ...
        for i in range(len(k_components[1])):
            ...
            segmentosDeVerdad.append(self.combinaSegmentos2(segmentos))
    ...
    def combina2(self,distancia1,distancia2,diffOrientacion,longcorto,lines,G): 
        ...
        for i in range(len(lines)-1):
            for j in range(i+1,len(lines)):
                d1 = self.segmentsDistance(lines[i],lines[j])
                d2 = self.segmentPoints_distance(lines[i],lines[j])
    ...
    # si están cerca comprueba si son consecutivas o no
    # epsilon para determinar si son paralelas o no
    # disSecantes
    # disParalelas
    # disConsecutivas
    # longCorto
    def combina3(self,disSecantes,disParalelas,disConsecutivas,epsilon, longcorto,lines,G): 
        for i in range(len(lines)):
            G.add_node(i)
        ...
        for i in range(len(lines)-1):
            for j in range(i+1,len(lines)):
                ...
                flag = isParallelAnConsecutive(lines[i], lines[j], epsilon)                
                dist = self.segmentsDistance(lines[i],lines[j])
                
                if flag == 0 and dist<disParalelas:
                    G.add_edge(i,j)
                elif flag == 1 and dist<disConsecutivas:
                    G.add_edge(i,j)
                elif flag == -1 and dist<disSecantes and (modulo(lines[i])<longcorto or modulo(lines[j])<longcorto):
                    G.add_edge(i,j)
